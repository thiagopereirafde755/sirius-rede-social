...
function recarregarComentarios(postId) {
    ...
    fetch(window.location.href, { cache: "reload" })
        ...
        .then(html => {
            // Cria DOM temporário para buscar o modal do post
            ...
            if (novaCommentInput && atualCommentInput) {
                ...
                // Se necessário, inicializa eventos ou plugins nos novos elementos
                atualCommentInput.querySelectorAll('textarea[name="comentario"]').forEach(setupMentionInput);
            }

            // ATUALIZA O CONTADOR DE COMENTÁRIOS (caso tenha)
...
function inicializarContadores() {
    document.querySelectorAll('textarea[name="comentario"]').forEach(function(input) {
        // Evita múltiplos contadores
        if (input._contadorAdicionado) return;
        input._contadorAdicionado = true;

        const limite = 300;
        // Cria ou encontra o elemento de contador
        let contador = input.parentNode.querySelector('.contador-caracteres');
        if (!contador) {
        ...
        limitarCaracteres(input, contador, limite);
        atualizarContador(input, contador, limite);
    });
}

// Chama ao carregar página e ao inserir novos campos dinâmica/JS
document.addEventListener('DOMContentLoaded', inicializarContadores);

// Sempre inicializa contador ao inserir novo textarea dinamicamente (ex: novo comentário ou resposta)
...
function toggleRespostas(button) {
    ...
    // Encontra todas as respostas para este comentário específico
    ...
    const respostas = Array.from(todasRespostas).filter(resposta => {
        // Verifica se é resposta direta ao comentário principal
        if (resposta.previousElementSibling === comentarioPrincipal) return true;
        
        // Verifica se é resposta de outra resposta deste comentário
...
function setupMentionInput(input) {
    if (!input || input._mentionSetup) return; // previne duplicidade
...
function insertMention(user) {
    ...
    // Fecha sugestões caso o input perca o foco
    input.addEventListener('blur', function() {
        setTimeout(() => suggestionsContainer.style.display = 'none', 150);
    });

    // Sempre reposiciona ao rolar página ou focar
    window.addEventListener('scroll', positionContainer);
    input.addEventListener('focus', positionContainer);
}

// Ativa autocomplete em todos os inputs já existentes ao carregar a página
...
function submitReply(event, postId, parentCommentId) {
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <form action="/apagar_comentario/${data.comentario_id}" method="POST" style="display:inline;">
                                <a href="#"  class="dropdown-item" data-comentario-id="${data.comentario_id}" onclick="apagarComentario(this)"> <i class='bx bx-trash'></i> Excluir comentario</a>
    ...
    <form class="form-resposta-comentario" onsubmit="submitReply(event, '${postId}', '${data.comentario_id}')">
        ...
        <button type="button" class="btn-cancelar-resposta" onclick="esconderFormResposta(this)">
            ...
            // Insere a nova resposta no local correto
            let insertedReply;
